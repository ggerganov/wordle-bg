...
bool ImGui_tryLoadFont(const std::string & filename, float size = 14.0f, bool merge = false) {
    ...
    printf(" - success\n");
    if (merge) {
        ImGui::GetIO().Fonts->AddFontFromFileTTF(filename.c_str(), size, NULL, ImGui::GetIO().Fonts->GetGlyphRangesCyrillic());
    } else {
...
int main(int argc, char** argv) {
    ...
    // Wordle video settings
    //float scale = 0.65;

    //int windowX = scale*570;
    //int windowY = scale*917;

    const char * windowTitle = "Wordle";
...

    {
        ...
        isNotLoaded = isNotLoaded && !ImGui_tryLoadFont(getBinaryPath() + "Alice-Regular.ttf", kGlobalImGuiScale*fontSize, true);
        isNotLoaded = isNotLoaded && !ImGui_tryLoadFont(getBinaryPath() + "../../assets/Alice-Regular.ttf", kGlobalImGuiScale*fontSize, true);
    }
